...
public interface SectionRepository extends JpaRepository<Section, Integer>  {
	...
	@Query("select section from Section section left join section.schoolClass schoolClass left join schoolClass.school school left join section.professor professor where (locate(lower(?1), lower(professor.name)) > 0 or locate(lower(?1), lower(section.name)) > 0 or locate(lower(?1), lower(schoolClass.name)) > 0) and school.id=?2")
